...
def run_simulation(
    ...
    Z: np.ndarray,
    NUM_ITER: int = 1_500_000,
    progress_interval: int = 10000,
):
    """
    Yields progress dicts during simulation, then final (positions, velocities) tuple.
    """
    initial_velocity = np.array([1.0, 1.0]) * 0.00002 # small initial velocity
    ...
    for i in range(NUM_ITER):
        ...
        if i > 0 and i % progress_interval == 0:
            yield {
                ...
                "progress_percent": (i / NUM_ITER) * 100,
                "trajectory_points": len(position_over_iterations_raw),
            }

    position_over_iterations = np.stack(position_over_iterations_raw)
    velocity_mag_over_time = np.stack(velocity_mag_over_time_raw) * 90 * 10 * 8
    yield (position_over_iterations, velocity_mag_over_time ** 1.5)
